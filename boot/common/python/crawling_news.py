...
def crawling_news1(soup):
    ...
    # 기자 구하기
    author_info = soup.find("a", class_="report-by")
    ...
    # 날짜 구하기
    date_info = soup.find("time", id="published")
    if date_info is not None:
        datetime_str = date_info["datetime"]  # '2024-07-05T14:58:34+09:00' 형식의 날짜와 시간을 가져옵니다.
        date = datetime_str[:10].replace('-', '')  # 날짜 부분만 추출합니다.
        time = datetime_str[11:16]  # 시간 부분만 추출합니다.
    else:
        date = ''
        time = ''

    # 본문 구하기
    content = soup.find("div", id="articleBodyContent")
    print(content)
    if content is not None:
        content = content.text.strip()
    else:
