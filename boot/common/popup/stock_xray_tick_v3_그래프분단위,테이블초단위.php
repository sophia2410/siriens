...
<body id="page-top">
    <div id="content">
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                labels: <?php echo json_encode($times_chart); ?>, // X축 데이터
                datasets: [{
                    label: '거래대금 (억)',
                    type: 'bar', // 이 데이터셋의 차트 유형을 막대로 설정
                    data: <?php echo json_encode(array_map(function($value) { return $value; }, $amounts_chart)); ?>,
                    ...
                    data: <?php echo json_encode($cum_amts_chart); ?>,
                    ...
                    label: '등락률 (%)',
                    type: 'line', // 이 데이터셋의 차트 유형을 선으로 설정
                    data: <?php echo json_encode($change_rates_chart); ?>,
            ...
            options: {
                scales: {
                    x: {
                        type: 'category',
                        labels: <?php echo json_encode($times_chart); ?>
                    },
                    y1: {
                        type: 'linear',
                        position: 'left',
