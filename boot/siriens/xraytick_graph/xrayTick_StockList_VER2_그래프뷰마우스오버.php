...
if($pgmId == '') {
	...
	foreach ($watchlist_data as $row) {
		...
		while($row = $result2->fetch_array(MYSQLI_BOTH)) {
			$xray_date .= "<th align=center style='width:80px; height: 30px;'><a href=\"javascript:void(0)\" onmouseover=\"showGraphPopup(event, '{$code}', '".$row['date']."')\" onmouseout=\"hideGraphPopup()\">". $row['date_str']."</a></th>";


			// $xray_date .= "<th align=center style='width:80px; height: 30px;'><a href=\"javascript:void(0)\" onmouseover=\"showGraphPopup(event, '{$code}', '".$row['date']."')\" onmouseout=\"hideGraphPopup()\" onclick=\"javascript:openPopupXrayTick('{$code}', '".$row['date']."')\">". $row['date_str']."</a></th>";
