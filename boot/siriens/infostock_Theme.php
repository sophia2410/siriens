...
<?php
	$query = " SELECT A.date, B.evening_report_title
				 FROM calendar A
	...
	while($row = $result->fetch_array(MYSQLI_BOTH)) {
		...
		$input_date .= "<option value='". $row['date']."' $select>". $row['date']." - ". $row['evening_report_title']."</option>";
	}
	$input_date .= "</select> ";
?>
...
<table style="width:100%;" class="table table-danger text-dark">
	<tr>
		<td colspan=2><?=$input_date?> <input type='button' id='searchbtn' class="btn btn-danger" onclick='searchReport()' value='조 회'></td>
	</tr>
	<tr>
		<td style='width:70%'>
			<div style="margin: 0; border: 0; font: inherit;vertical-align: baseline; padding: 0;height: calc(100vh - 155px);">
				<iframe id="iframeL" style="width: 100%; margin: 0; border: 0; font: inherit; vertical-align: baseline; padding: 0; height: calc(100vh - 155px);" src="infostock_Theme_L.php">
				</iframe>
			</div>
		</td>
		<td>
			<div style="margin: 0; border: 0; font: inherit;vertical-align: baseline; padding: 0;height: calc(100vh - 155px);">
				<iframe id="iframeR" style="width: 100%; margin: 0; border: 0; font: inherit; vertical-align: baseline; padding: 0; height: calc(100vh - 155px);" src="infostock_Theme_R.php">
				</iframe>
			</div>
		</td>
	</tr>
...
function searchReport() {
	...
	iframeL.src = "infostock_Theme_L.php?infostock_date="+key_val;
	iframeR.src = "infostock_Theme_R.php?infostock_date="+key_val;
	return;
}
</script>
</body>
