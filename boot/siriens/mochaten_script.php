	...
	if($_POST['proc_fg'] == 'save') {	// mochaten_B.php
		...
		for($i=0; $i<$_POST['cnt']; $i++){
			...
			if(isset($_POST[$check])) {
				...
				$qry = "REPLACE INTO  siriens_report_stock (report_date, code, name)
						SELECT '".$_POST[$trade_date]."'
						 ...
						 WHERE code = '".$_POST[$code]."'
						   AND last_yn = 'Y'
						   AND NOT EXISTS (SELECT * FROM siriens_report_stock Z WHERE Z.report_date = '".$_POST[$trade_date]."' AND Z.code = A.code)";

				echo $qry."<br>";
				$mysqli->query($qry);

				for($j=0; $j<count($keyword); $j++) {
					$qry = "REPLACE INTO  siriens_report_keyword (report_date, code, keyword_cd, create_dtime)
		...
		for($i=0; $i<$_POST['cnt']; $i++){
			...
			if(isset($_POST[$check])) {
				if(count($keyword) == 0) {
					//키워드 삭제
					$qry = "DELETE FROM siriens_report_keyword
					...
					//today stock 삭제
					$qry = "DELETE FROM siriens_report_stock
							WHERE report_date = '".$_POST[$trade_date]."'
							AND code = '".$_POST[$code]."'";
