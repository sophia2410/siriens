...
if($report_date == '') {
	...
	$query = "SELECT '1' regi_fg FROM siriens_evening WHERE report_date = '$report_date' LIMIT 1";
	$result = $mysqli->query($query);
	$row = $result->fetch_array(MYSQLI_BOTH);

	// siriens_evening 테이블에 데이터 생성
	if(!isset($row['regi_fg'])) {
		...
		if(isset($row['regi_fg'])) {
			...
			$qry = "INSERT INTO siriens_evening (report_date, code, name, stock_CPR, issue, keyword, stock_keyword)
...
<iframe name="iframeH" src = "siriensEvening_script.php" width=100% height=100>
</html>