...
// 테마별 종목 조회 건수
$detail_cnt = (isset($_GET['detail_cnt']) && $_GET['detail_cnt'] != '') ? $_GET['detail_cnt'] : 10;
...
if($specific_datetime == '') {
		...
		FROM (
			SELECT
				...
				WHERE m2.code = m.code AND 
					  STR_TO_DATE(CONCAT(REPLACE(m2.date, '-', ''), m2.minute), '%Y%m%d%H%i') <= t.specific_datetime
	...
	foreach ($tableData as $tableName => $themes) {
		...
		echo "<th width=85>3분전</th>";
		echo "<th width=85>4분전</th>";
		echo "<th width=85>5분전</th>";
		echo "</tr>";
