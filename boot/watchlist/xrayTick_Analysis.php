...
<div class="flex-container">
    <div class="left">
        ...
        <input type="date" id="search_date" value="<?= date("Y-m-d") ?>">
...
$(document).ready(function() {
    // Event listener for the "최근테마(옵시디언)" button
...
// Function to hide and clear the theme container
function resetThemeContainer() {
    hideSectorContainer(); // Always hide the container first
    $('#sectorContainer').empty(); // Clear previous contents
}

// Function to hide the sector container only
function hideSectorContainer() {
...
function fetchThemes(source) {
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#sectorContainer').show(); // Show the container after loading new content
        },
...
function viewStockList(pgmId, key1='', key2='') {
    ...
    } else {  // pgmId == 'mochaten', 'IPOstock', 'buyStreakMonthly'
        parm = "";
    }
...
// 현재 URL에서 쿼리스트링 파라미터를 가져오는 함수
function getCurrentParams() {
    // iframeB.src에서 파라미터만 가져오기
    let currentUrl = iframeB.src;
    let params = currentUrl.split('?')[1]; // ? 뒤의 파라미터 부분만 가져옴
    return params ? '?' + params : ''; // 파라미터가 있으면 붙이고, 없으면 빈 문자열
}
