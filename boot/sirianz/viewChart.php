...
if($pgmId == ''){
	...
	if($pgmId == 'kiwoomapi_opt10029') {
		$grpqry= "SELECT CONCAT(' [ ' , K.opt10029_fg, '-', S.market_fg,' ] ') AS group_key
						,CONCAT(' [ ' , K.opt10029_fg, '-', S.market_fg,' ] ') AS group_val
					FROM 
					(	SELECT '224' opt10029_fg
						UNION ALL
						SELECT 'ALL' opt10029_fg ) K
					INNER JOIN 
					(	SELECT 'KOSPI' market_fg
						UNION ALL
						SELECT 'KOSDAQ' market_fg ) S
					ORDER BY K.opt10029_fg, S.market_fg";
