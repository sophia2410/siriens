...
if(isset($_POST['proc_fg'])) {	
	...
	// 시리언즈 이브닝 종목 삭제
	else if($_POST['proc_fg'] == 'stockDel') {	// sirianzEvening_B.php
		for($i=0; $i<$_POST['cnt']; $i++){
			$check	= 'check'.$i;
			$id		= 'id'.$i;
	...
	// 테마 정보 등록,수정
		...
		for($i=0; $i<$_POST['cnt']; $i++){
			...
			if($_POST[$theme_tb_id] == 'NoData') {
				$qry = "INSERT INTO sirianz_evening_theme (report_date, theme, issue, keyword, signal_id1, signal_id2, signal_id3, create_id, create_dtime)
						VALUES ('".$_POST['report_date']."', '".$_POST[$theme]."', '".$_POST[$issue]."', '".$_POST[$keyword]."',
								 ...
								 'SirianzEvening_script', now())";
				echo $qry."<br>";
				$mysqli->query($qry);

			} else {
				$qry = "UPDATE sirianz_evening_theme
						SET theme		= '".$_POST[$theme]."'
						,	issue		= '".$_POST[$issue]."'
						, 	keyword		= '".$_POST[$keyword]."'
		...
		$qry = "UPDATE sirianz_evening_theme A
		...
		$qry = "DELETE FROM sirianz_evening_theme
				WHERE report_date ='".$_POST['report_date']."'
				AND	  dup_check != 'O'";
		echo $qry."<br>";
		$mysqli->query($qry);
