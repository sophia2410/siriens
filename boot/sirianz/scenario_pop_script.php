...
if(isset($_POST['proc_fg'])) {
	if($_POST['proc_fg'] == 'updateThemeOne') {
		...
		if($_POST['sector'] != $_POST['org_sector'] || $_POST['theme'] != $_POST['org_theme']) {
			...
			echo '<script>
					window.parent.location.href="http://localhost/boot/sirianz/scenario_PopT.php?sector='.$mod_sector.'&theme='.$mod_theme.'";
		...
		echo '<script>
				window.parent.location.href="http://localhost/boot/sirianz/scenario_PopT.php?sector='.$sector.'&theme='.$theme.'";
				window.parent.reload ();
			</script>';
	}
}
