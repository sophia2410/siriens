...
if(isset($row['date']) AND ($now >= '09:00:0' && $now <= '19:00:00')) {
...
if($pgmId == ''){
	...
	// $query = "SELECT max(date) base_date FROM calendar WHERE date <=(SELECT DATE_FORMAT(DATE_ADD(now(), INTERVAL 0 DAY), '%Y%m%d'))";
	
	// 장 시작전에는 전일 시세 가져오기 위해..
	$query = "
	SELECT max(date) base_date 
	FROM calendar 
	WHERE date <= (
		SELECT DATE_FORMAT(
			DATE_ADD(now(), INTERVAL IF(CURTIME() < '09:00:00', -1, 0) DAY), 
			'%Y%m%d'
		)
	)";
	...
	if($pgmId == 'kiwoomapi_opt10029') {
		$filename = $pgmId."_예상체결";
		$file_orderby = "ORDER BY V.opt10029_fg, V.market_fg, V.trade_rate_str DESC";
		$query = "SELECT CONCAT(' [ ' , A.opt10029_fg, '-', S.market_fg,' ] ') AS group_key
						, A.opt10029_fg
						, S.market_fg
						, S.code 
						, S.name
						...
						, B.tot_trade_amt
						, CONCAT('*',  DATE_FORMAT(concat(A.date, A.sell_remain), '%m/%d %H:%i')) trade_date
					...
					FROM (	SELECT 
							...
							FROM 
								...
								FROM 
									daily_price 
								WHERE
									date > DATE_FORMAT('$base_date', '%Y%m%d') - INTERVAL 224 DAY AND
									code IN (SELECT code FROM kiwoom_opt10029 WHERE date = '$base_date')
								GROUP BY 
									code
								) AS dp
									JOIN 
										kiwoom_opt10029 AS k
									ON 
										dp.code = k.code
									WHERE 
										k.date = '$base_date' AND
										((k.base_price < dp.avg_close AND k.exp_price >= dp.avg_close) OR 
										k.exp_price BETWEEN dp.avg_close * 0.97 AND dp.avg_close * 1.03)
							UNION ALL
							SELECT 
								'ALL' opt10029_fg,
								date, 
					...
					ON S.code = A.code
					LEFT OUTER JOIN (SELECT * FROM watchlist_sophia WHERE (CONCAT(sector,theme,category), code) IN (SELECT MIN(CONCAT(sector,theme,category)), code FROM watchlist_sophia  GROUP BY code)) W
					ON W.code = A.code
					...
					ON M.code = A.code
					ORDER BY A.opt10029_fg, S.market_fg, A.rate DESC";
	} else if($pgmId == 'kiwoomapi_realtime') {
		$filename = $pgmId."_실시간대량체결";
		$file_orderby = "ORDER BY SUBSTR(A.first_time,1,4) DESC, A.acc_volume DESC";
		$query = "SELECT '실시간대량체결포착' group_key
						, A.first_time
						, S.market_fg
						, S.code 
						, S.name
						...
						, CASE WHEN B.tot_trade_amt >= 1000 THEN CONCAT('<font color=red><b>',FORMAT(B.tot_trade_amt,0),'억</b></font>') ELSE  CONCAT(B.tot_trade_amt,'억') END tot_trade_amt_str
						, B.tot_trade_amt
						, CONCAT('*',  DATE_FORMAT(concat(A.date, A.first_time), '%m/%d %H:%i')) trade_date
						, A.price trade_price
						, CASE WHEN A.rate >= 0 THEN CONCAT('<font color=red> +',A.rate,'% </font>') ELSE  CONCAT('<font color=blue> -',ABS(A.rate),'% </font>') END trade_rate_str
						, ROUND(A.acc_trade_amount/100) acc_trade_amount
						, M.mochaten_cnt
					FROM kiwoom_realtime A
					INNER JOIN kiwoom_stock S
					ON S.code = A.code
					LEFT OUTER JOIN (SELECT * FROM watchlist_sophia WHERE (CONCAT(sector,theme,category), code) IN (SELECT MIN(CONCAT(sector,theme,category)), code FROM watchlist_sophia  GROUP BY code)) W
					ON W.code = A.code
					...
					LEFT OUTER JOIN (SELECT code, count(*) mochaten_cnt FROM mochaten WHERE cha_fg = 'MC000' GROUP BY code) M
					ON M.code = A.code
					WHERE A.date = '$base_date'
					AND S.market_fg in ('KOSPI', 'KOSDAQ')
					AND S.name not like '%ETN%'
					ORDER BY SUBSTR(A.first_time,1,4) DESC, A.acc_volume DESC";
		...
		$query = "SELECT RTRIM(CONCAT(' [ ' , A.sector,' ] ', A.theme, '  ',A.category)) AS group_key
					...
					ORDER BY A.sector, A.sort_theme, acc_trade_amount DESC, A.sort_stock";
	...
	while($row = $result->fetch_array(MYSQLI_BOTH)) {
		...
		$realtime_data = "";
		if($row['trade_date'] != '') {
			$realtime_data = "<font class='h5'>".$row['trade_rate_str']." </font> &nbsp";
			$realtime_data .= "<font class='text-dark'>".number_format($row['trade_price'])."&nbsp ".number_format($row['acc_trade_amount'])."억  &nbsp ".$row['trade_date']."</font> ";
		}
