...
<body>
<form name="form1" method='POST' action='sirianzTrade_script.php'>
<?php
	if($trade_date == ''){
		echo "<h3></h3>";
	}else if($code == ''){
		...
		// Daily Trade 코멘트
		$query = "SELECT regi_id, trade_date, daily_comment, good_comment, bad_comment, trade_rate, buysell_fg FROM sirianz_trade WHERE trade_date=$trade_date AND code='$code' AND regi_id = 'sophia'";
		$result = $mysqli->query($query);
...
<iframe name="saveComment" src="sirianzTrade_script.php" width=0 height=0>
</iframe>
</body>
<script>
