...
<body>
<form name="form1" method='POST' action='hotStock_script.php'>
<?php
	if($report_date == ''){
		echo "<h3></h3>";
	}else if($code == ''){
		...
		$query = "SELECT  CONCAT(CASE WHEN length(C.today_theme_nm) > 1 THEN CONCAT('[',C.today_theme_nm,'] ',C.issue,'<BR><BR>') ELSE '' END) today_theme
					...
					WHERE B.report_date = (select max(date) from calendar where date < '$report_date')
					AND B.code =  '$code'" ;
		// echo "<pre>$query</pre>";
		$result = $mysqli->query($query);
		...
		// if($today_issue == '') {
			$query = "SELECT CONCAT('[',A.signal_grp
						...
						WHERE	page_date = (select max(date) from calendar where date < '$report_date')
						AND  page_fg = 'E'
						AND  today_pick = 'Y'
						AND  stock =  '$code'" ;
		...
		if($today_issue == '') {
			$query = "SELECT  date
						...
						FROM signals B
						WHERE B.code =  '$code'
						AND B.date <= (select max(date) from calendar where date < '$report_date')
						ORDER BY date DESC
						LIMIT 1 " ;
...
<iframe name="saveComment" src="hotStock_script.php" width=1000 height=100>
</iframe>
</body>
<script>
