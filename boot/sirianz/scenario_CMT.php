...
$query = " SELECT STR_TO_DATE(A.date, '%Y%m%d') issue_date, DAYOFWEEK(A.date) - 2 arr_idx
			...
			LEFT OUTER JOIN (SELECT watchlist_date
									, GROUP_CONCAT('<br>', CASE WHEN hot_theme = 'Y' THEN CONCAT('<b><font color=red>',today_theme,'</font></b>') ELSE today_theme END ORDER BY hot_theme DESC, tot_trade_amt DESC) today_theme
								FROM (SELECT watchlist_date, hot_theme
										, CASE WHEN theme is null OR  theme = '' THEN sector ELSE theme END today_theme
...
function search() {
	...
	document.forms[0].action = "./scenario_CMT.php?issue_mon="+issue_mon;
