...
<?php
	$query = " SELECT A.date, B.evening_subject, IF(C.proc_cnt>0,'Y', 'N') proc_yn
				 ...
				 LEFT OUTER JOIN (SELECT report_date, COUNT(*) proc_cnt FROM sirianz_infostock WHERE report_date <= DATE_ADD(now(), INTERVAL 0 DAY) GROUP BY report_date ORDER BY report_date desc LIMIT 100) C
