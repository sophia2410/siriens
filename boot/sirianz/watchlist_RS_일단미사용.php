...
<body>
<form name="form1" method='POST' action='watchlist_script.php'>
<?php
	if($watchlist_date == ''){
		echo "<h3></h3>";
	} else {
		...
		echo "<table style='width:100%;'>
				...
				<td width=85%'>";
					...
					$query = "	SELECT STR_TO_DATE(A.watchlist_date, '%Y%m%d') watchlist_date_str
									...
									, A.volume
									, A.market_cap
									, CASE WHEN A.sector IS NOT NULL THEN A.sector ELSE (SELECT sector FROM sophia_watchlist WHERE watchlist_date < '$watchlist_date' AND code = A.code ORDER BY watchlist_date DESC LIMIT 1) END sector
									, CASE WHEN A.theme IS NOT NULL THEN A.theme ELSE (SELECT theme FROM sophia_watchlist WHERE watchlist_date < '$watchlist_date' AND code = A.code ORDER BY watchlist_date DESC LIMIT 1) END theme
									, CASE WHEN A.issue IS NOT NULL THEN A.issue ELSE (SELECT issue FROM sophia_watchlist WHERE watchlist_date < '$watchlist_date' AND code = A.code ORDER BY watchlist_date DESC LIMIT 1) END issue
									, CASE WHEN A.stock_keyword IS NOT NULL THEN A.stock_keyword ELSE (SELECT stock_keyword FROM sophia_watchlist WHERE watchlist_date < '$watchlist_date' AND code = A.code ORDER BY watchlist_date DESC LIMIT 1) END stock_keyword
									, A.tracking_yn
									, A.tracking_reason
					...
					echo "<tr align=left>";
					echo "<td colspan=2><b>$watchlist_date_str</b>&nbsp;&nbsp;&nbsp;&nbsp;";
					...
					echo "</tr>";
					echo "<tr align=left>";
					echo "<td style='width:110px;'> 트래킹 </td>";
					...
					// 다음 영업일 시나리오 보기
					$query = "	SELECT STR_TO_DATE(B.scenario_date, '%Y%m%d') scenario_date_str
									, B.scenario_date
								...
								RIGHT OUTER JOIN (SELECT MIN(date) scenario_date FROM calendar WHERE date >= now()) B
								ON A.scenario_date = B.scenario_date
								WHERE A.code = '$code'";
					...
					// 시나리오 작성 부분
					echo "<tr align=left>";
					echo "<td style='width:110px;'> 매매고려 </td>";
					echo "<td>$buy_pick</td>";
					echo "<td>(예상이평가)</td>";
					...
					echo "<td> 시나리오</td>";
					echo "<td colspan=4 style='height:80px;'><textarea name='scenario' style='width:99%; height:80px;'>$scenario</textarea></td>";
					echo "</tr>";
					echo "<tr align=left style='background-color:white'>";
					echo "<td colspan=7></td>";
					echo "</tr>";
					...
					while($row = $result->fetch_array(MYSQLI_BOTH)) {
						...
						echo "차트 : ".$row['mavg'].$row['buy_pick']."</td>";
						echo "</tr>";
						echo "<tr>";
		...
		echo "	</td>
				...
				<div style='margin: 0; border: 0; font: inherit;vertical-align: baseline; padding: 0;width=20%; height: calc(100vh - 500px);'>
					<iframe id='iframe' style='width: 100%; margin: 0; border: 0; font: inherit; vertical-align: baseline; padding: 0; height: calc(100vh - 500px);' src='watchlist_RS_S.php'>
					</iframe>
				</td>
			</tr>
		</table>";
...
<iframe name="saveFrame" src="watchlist_script.php" style='border:0px;' width=1000 height=700>
</iframe>
</body>
...
// 입력 종목 추가하기
function addData(date, stock) {
	form = document.form1;
	form.proc_fg.value = 'addData';
	form.watchlist_date.value = date;
