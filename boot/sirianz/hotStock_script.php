...
if(isset($_POST['regi_id']) && $_POST['regi_id'] != '') {
	...
	$qry = "DELETE FROM mochaten_comment WHERE regi_id = '".$_POST['regi_id']."' and report_date='".$_POST['report_date']."' AND code='".$_POST['code']."' ";
	// echo $qry;
	$result = $mysqli->query($qry);
	
	if($geo.$cha.$jae.$si.$chart_comment != "" || $chart_grade != "") {
		$qry = "INSERT INTO mochaten_comment (report_date, code, regi_id, chart_comment, chart_grade, chart_pick, geo, cha, jae, si) 
					VALUES ('".$_POST['report_date']."', '".$_POST['code']."', '".$_POST['regi_id']."', '$chart_comment', '$chart_grade', NULL, '$geo', '$cha', '$jae', '$si')";
		echo $qry;
		$result = $mysqli->query($qry);
	}
	...
	if($_POST['proc_fg'] == 'save') {	// mochaten_B.php
		...
		$qry = "UPDATE mochaten
				 ...
				 WHERE report_date='".$day0_date."' AND code='".$_POST['code']."'";
		echo $qry."<br><br>";
		$mysqli->query($qry);
		...
		for($i=0; $i<$_POST['keyword_cnt']; $i++){
			...
			$qry = "UPDATE mochaten
					 ...
					 WHERE report_date ='".$_POST[$report_date]."' AND code='".$_POST[$code]."'";
			echo $qry."<br>";
			$mysqli->query($qry);
		}
		...
		for($i=0; $i<$_POST['cnt']; $i++){
			...
			if(isset($_POST[$check])) {
				...
				$qry = "REPLACE INTO  sirianz_report_stock (report_date, code, name)
						SELECT '".$_POST[$report_date]."'
							 , A.code
							 , A.name
						  FROM stock A
						 WHERE code = '".$_POST[$code]."'
						   AND last_yn = 'Y'
						   AND NOT EXISTS (SELECT * FROM sirianz_report_stock Z WHERE Z.report_date = '".$_POST[$report_date]."' AND Z.code = A.code)";
				...
				for($j=0; $j<count($keyword); $j++) {
					$qry = "REPLACE INTO  sirianz_report_keyword (report_date, code, keyword_cd, create_dtime)
							VALUES ('".$_POST[$report_date]."', '".$_POST[$code]."', '".$keyword[$j]."', NOW()) ";
