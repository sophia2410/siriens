...
while($row = $result->fetch_array(MYSQLI_BOTH)) {
	...
	if($pre_date != $row['report_date']) {
		...
		$input[$r3][$arr_idx] .= "<a href='../sirianz/stock_B.php?code=".$row['code']."&name=".$row['name']."' class='"."$stock_style"."' onclick='window.open(this.href, 'stock', 'width=1800px,height=850,scrollbars=1,resizable=yes');return false;' target='_blank'>".$row['name']."</a></td>";
...
function search() {
	...
	document.forms[0].action = "./sirianzSummary.php?report_mon="+report_mon+"&tenbillion="+tenbillion+"&tenbillionOR="+tenbillionOR+"&onlyTitle="+onlyTitle;
