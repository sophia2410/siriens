...
$query = " SELECT A.mochaten_date
			...
			INNER JOIN comm_cd B
			 ...
			 LEFT OUTER JOIN (SELECT code, '(V)' chart_grade FROM mochaten_comment WHERE regi_id = 'nomad' AND mochaten_date = '$mochaten_date') F
			   ON F.code = A.code
		     LEFT OUTER JOIN (SELECT code, close_rate FROM daily_price WHERE date = '$mochaten_date') E
			   ON E.code = A.code
	...
	if($browser_width >=1500) {
		while($row = $result->fetch_array(MYSQLI_BOTH)) {
			if($pre_cha_fg != $row['cha_fg']) {
				echo "<tr class='table-primary  text-dark' align=center><th colspan=2><b>".$row['cha_fg_nm']."</b></th><th>소피아</th><th>시스터</th><th>등락률</th></tr>";
			}
			echo "<tr><td style='width:10px;'>".$row['nomad_comment_fg']."</td>";
			echo "<td><a href=\"javascript:callFrameR('".$mochaten_date."','".$row['code']."')\">".$row['name']."</a></td>";
		...
		while($row = $result->fetch_array(MYSQLI_BOTH)) {
			if($pre_cha_fg != $row['cha_fg']) {
				echo "<tr class='table-primary  text-dark' align=center><th><b>".$row['cha_fg_nm']."</b></th></tr>";
			}
			echo "<tr><td><a href=\"javascript:callFrameR('".$mochaten_date."','".$row['code']."')\">".$row['name']."</a></td>";
			echo "</tr>" ;
...
function callFrameR(date, cd) {
	window.parent.viewMochaten(date, cd);
}
</script>
</html>
