...
<head>
  ...
  <style>
    ...
    .logBox{height:110px;overflow:auto;}
...
<div class="gridTop">
  ...
  <div class="panel logWrap">
    ...
    <textarea id="dayComment" rows="16" spellcheck="false" autocorrect="off" autocapitalize="off" autocomplete="off"
...
function colorBSCandles(chart, buyMap, sellMap){
  const candle = chart?.series?.[0];
  if (!candle) return;
...
console.log('candle x sample', chart.series[0].xData.slice(0, 10).map(x => Highcharts.dateFormat('%H:%M', x)));
    colorBSCandles(chart, buyMap, sellMap);
