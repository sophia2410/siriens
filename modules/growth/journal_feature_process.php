...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $journal_date = $mysqli->real_escape_string($_POST['journal_date']);
    ...
    if (!empty($_POST['journal_id'])) {
        ...
        $updateQuery = "UPDATE journal_feature 
                        SET code = '$stock_code', journal_date = '$journal_date', type = '$type', status = '$status', comment = COMPRESS('$comment')
                        WHERE id = '$journalId'";
        $mysqli->query($updateQuery);
    } 
    // 새로운 매매/복기 등록하는 경우
    else {
        $insertQuery = "INSERT INTO journal_feature (code, journal_date, type, status, comment) 
                        VALUES ('$stock_code', '$journal_date', '$type', '$status', COMPRESS('$comment'))";
        $mysqli->query($insertQuery);
    }

    // 처리 완료 후 검색 조건과 함께 리다이렉트
    header("Location: journal_feature_register.php?journal_date=$journal_date&type=$type&stock_name=$searchStockName&search_type=$searchType&search_status=$searchStatus&page=" . $page);
}
elseif ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    if ($_GET['action'] == 'delete' && isset($_GET['journal_id'])) {
        $journalId = $mysqli->real_escape_string($_GET['journal_id']);
        $deleteQuery = "DELETE FROM journal_feature WHERE id = '$journalId'";
        $mysqli->query($deleteQuery);

        // GET으로 받은 검색 조건
        $journal_date = isset($_GET['journal_date']) ? $_GET['journal_date'] : date('Y-m-d');
