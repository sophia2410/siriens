...
$type = isset($_GET['type']) ? $_GET['type'] : 'reflection';
...
$journalQuery = "
    SELECT tj.id, tj.trade_date, tj.code, s.name, tj.type, tj.status, 
        CASE 
            ...
            ELSE '' 
        END AS status_str,       
        UNCOMPRESS(tj.comment) AS comment, tj.created_at
...
// 종목명 검색 조건 추가
if ($searchStockName) {
    $whereConditions[] = "(s.name LIKE '%$searchStockName%' OR tj.code LIKE '%$searchStockName%')";
}

// 구분 검색 조건 추가
if ($searchType) {
    $whereConditions[] = "tj.type = '$searchType'";
}

// 상태 검색 조건 추가
if ($searchStatus) {
    $whereConditions[] = "tj.status = '$searchStatus'";
}
...
// 정렬과 페이징 처리
$journalQuery .= " ORDER BY tj.id DESC LIMIT 2 OFFSET " . ($page - 1) * 2;
...
<body>
    <div id="container">
        ...
        <div id="journal_register_container">
            <h2>매매/복기 등록</h2>
            <form id="journalForm" action="trade_journal_process.php" method="POST" onsubmit="return validateForm();">
        ...
        <div id="journal_list_container">
            ...
            <?php while ($row = $journals->fetch_assoc()) { ?>
                <div class="journal-card">
                    ...
                    <?php if (!in_array($row['type'], ['trade', 'reflection'])) { ?>
                        <div onclick="loadJournalData(<?= $row['id']; ?>)">
                            <img class='img-fluid' width=550 src="https://ssl.pstatic.net/imgfinance/chart/item/candle/day/<?= $row['code'] ?>.png?sidcode=1705826920773">
                            <img class='img-fluid' width=550 src="https://ssl.pstatic.net/imgfinance/chart/item/candle/month/<?= $row['code'] ?>.png?sidcode=1705826920773">
                        </div>
                    <?php } ?>
    ...
    <script>
        ...
        function deleteJournal() {
            if (confirm('정말로 이 매매/복기를 삭제하시겠습니까?')) {
                ...
                window.location.href = "trade_journal_process.php?action=delete&journal_id=" + journalId + "&page=<?php echo $page; ?>";
            }
        }
