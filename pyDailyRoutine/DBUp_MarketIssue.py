...
def handle_keywords(cursor, keywords):
    ...
    for keyword in keyword_list:
        ...
        cursor.execute("SELECT keyword_id FROM keyword_master WHERE keyword = %s", (keyword,))
        result = cursor.fetchone()
        if result:
            keyword_ids.append(result[0])
        else:
            cursor.execute("INSERT INTO keyword_master (keyword) VALUES (%s)", (keyword,))
    ...
    if result:
        ...
        group_name = keywords
        cursor.execute("INSERT INTO keyword_groups_master (group_name) VALUES (%s)", (group_name,))
        group_id = cursor.lastrowid

        for keyword_id in keyword_ids:
            cursor.execute("INSERT INTO keyword_groups (group_id, keyword_id, create_dtime) VALUES (%s, %s, NOW())", (group_id, keyword_id))
        
        print(f"Created new keyword group: {group_name} with ID: {group_id}")
        return group_id

# 이슈 삽입 및 업데이트 함수
def insert_or_update_issue(cursor, date, issue, first_occurrence, link, sector, theme, hot_theme, group_id):
    print(f"Inserting/updating issue: {issue} for date: {date}, sector: {sector}, theme: {theme}")

    cursor.execute("""
        SELECT issue_id FROM market_issues 
        WHERE date = %s AND keyword_group_id = %s AND issue = %s AND sector = %s AND theme = %s
    """, (date, group_id, issue, sector, theme))
    result = cursor.fetchone()
    ...
    if result:
        ...
        cursor.execute("""
            INSERT INTO market_issues (date, issue, first_occurrence, link, sector, theme, hot_theme, keyword_group_id, status, create_dtime) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'registered', NOW())
        """, (date, issue, first_occurrence, link, sector, theme, hot_theme, group_id))
        issue_id = cursor.lastrowid
        print(f"Inserted new issue with ID: {issue_id}")
        return issue_id

# 주식 데이터 처리 함수
def handle_stocks(cursor, issue_id, stocks, date):
    ...
    for stock in stocks:
        ...
        """
        params = (issue_id, code, name, close_rate, volume, trade_amount, stock_comment, is_leader, date)
...
def process_issues(db, df):
    ...
    try:
        for index, row in df.iterrows():
            ...
            # 키워드가 있는 경우에만 새로운 그룹으로 처리, 섹터와 테마는 비어있어도 상관없음
            if row['키워드']:
                keywords = row['키워드']
                theme = row['테마']
                sector = row['섹터']
                ...
                issue_id = insert_or_update_issue(cursor, date, issue, first_occurrence, '', sector, theme, hot_theme, group_id)
            ...
            handle_stocks(cursor, issue_id, stocks, date)
